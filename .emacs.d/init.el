...
(defvar orgit-base-url "https://github.com/steve-downey/backtick/blob/main/"
...
;; Pinned transclusion for blog posts.
;;
;; Two link types, two policies, carried over from the sibling repositories:
;;   - living docs transclude via `file:' / `orgit:' and roll forward with the
;;     worktree, which is the point of them;
;;   - blog posts transclude via `orgit-file:' pinned to a tag or commit, so a
;;     later change cannot rewrite the code inside a published entry.
;;
;; Both adapters live on `org-transclusion-add-functions' and coexist; the link
;; type selects the policy.
