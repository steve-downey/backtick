...
The other thing worth reporting is that both flag gates leaked. Clang's
=-fbacktick= reached the language options in a C compilation and made an invalid
C program compile; GCC's two backtick cases in the parser were fall-through
targets keyed on the flag instead of the token, so a program containing no
backtick could be diagnosed differently with the flag on. Both were one line,
and neither showed up in a diagnostic. The check that finds them is comparing
...
members, =typedef= names, the qualified name in an out-of-class definition) and
in expression positions, including after =.=. Neither takes it yet in a
class-head, an enum name, a namespace name, or a template parameter name, and
Clang takes it in an alias-declaration where GCC doesn't. The type-name slot is
Clang-only so far: GCC parses its slot as an expression, so =1 `Pt` 2= is
rejected there. Those are unwritten parser arms rather than design positions,
but a paper has to say which arms it has.
