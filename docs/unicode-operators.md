...
**Log.** 2026-09-05 — retired the serial number in favour of this slug; wording unchanged. 2026-09-09 — **the derived-table alternative was raised by the design author and declined, and its reasons are stronger than cost.** Deriving a per-code-point precedence in the standard, rather than letting a user declare one (Julia's many precedence classes; OCaml's fixity taken from an operator's first character), avoids every travel problem above and keeps the parse a function of the expression alone. It fails on two counts. **Nothing in Unicode supports the derivation.** Pattern_Syntax partitions syntax from identifiers and asserts nothing about meaning, blocks record allocation order, and the two distinctions a reader would actually want are adjacent code points: ⊕ ⊖ ⊗ ⊘ ⊙ are U+2295 through U+2299, and ∩ and ∪ are U+2229 and U+222A. Every mechanical derivation available (block, range, shape family, or the 32 contiguous ranges [token-set](#token-set) is already built from) collapses exactly the pairs it would have to separate, so the derivation is anti-correlated with the answer, and a real table would have to be curated code point by code point — the same hand-curated normative list [unary-forms](#unary-forms) priced and declined for postfix. **And a precedence is a meaning.** This design allocates notation and leaves semantics to the declaration; a table would have the committee assert that ⊗ is multiplication-like, which is true in tensor algebra, false in a monoidal category whose product is written ⊕, and backwards in a tropical semiring where ⊞ is addition and ⊙ is multiplication. C++ has exactly one precedent for a fixed precedence over a user-chosen meaning, and it is the language's most-cited precedence bug: `operator<<` inherited shift precedence, so `std::cout << a & b` is `(std::cout << a) & b`. Mathematics has no global table to copy either; it groups by two-dimensional layout and by convention set within a single paper. **Recorded as a concession, because a reviewer will raise it:** one level is not neutral. `a ⊕ b ⊗ c` groups as `(a ⊕ b) ⊗ c` and a reader out of the tensor literature expects the other, so fixed fixity does not avoid surprising that reader; it makes the surprise uniform and learnable, and leaves disambiguation in parentheses, which is where mathematics leaves it. 2026-09-09 — **the mixed-chain diagnostic is a tool question rather than a language one** (design author's ruling): a chain mixing distinct user operators is normal code with a reasonable reading, unlike assignment in a condition, so a compiler warning would fire forever on correct code whose author knows the rule, with "add parentheses you did not want" as its only suppression. That is the `-Wparentheses` boolean failure mode. Upstream has already settled it in that direction: clang-tidy ships `readability-math-missing-parentheses`, off by default, and its matcher excludes `&&` and `||` by name. A check for this feature would be a sibling rather than an extension of it, since that one fires when precedences *differ* while this one would fire on equal precedence with differing operators, over `UserOperatorExpr` / `BacktickInfixExpr` rather than `BinaryOperator`. The source-fidelity wrapper is what makes such a check writable at all: the semantic form is a nested call, and a nested call does not remember having been written infix. 2026-09-23 — **the derived-table alternative was raised again, from outside, and the 2026-09-09 entry above does not answer it.** Robin Leroy (@eggrobin.bsky.social) pointed at MathClass-15 (`https://www.unicode.org/Public/math/revision-15/MathClass-15.txt`), the data file behind UTR #25 — the report [token-set](#token-set) already draws the operator repertoire from — and proposed putting class R, *relation*, at a level near `<=>`/`==`. Measured against the frozen 1,381: **R 644 (46.6%), B *binary* 153 (11.1%), other 238, no entry 346 (25.1%)**. So "nothing in Unicode supports the derivation" is false as written, P4345R0 ships with it, and R1 must repair the passage. **The paper refutes a cut nobody proposed.** ⊕ ⊖ ⊗ ⊘ ⊙ are all class B, and ∩ and ∪ are both class B — MathClass agrees that no derivation separates *those*. Leroy's cut is R against B, which the data does make, over 58% of the set. **Do not answer on coverage:** 342 of the 346 unclassified code points sit in Miscellaneous Technical and Miscellaneous Symbols and Arrows, and coverage is 100% in Mathematical Operators, Supplemental Mathematical Operators, Miscellaneous Mathematical Symbols-A and -B, and both Supplemental Arrows blocks — it holds exactly where a mathematician reaches. Three answers survive. **MathClass is not a property:** its own header says it is "*NOT* formally part of the Unicode Character Database at this time" and that the information "will change from time to time"; revision 15 is dated 2017-06-01 over a Unicode 9.0 repertoire, while [token-set](#token-set) is frozen against UCD 17.0.0. Either [expr] tracks a file Unicode calls provisional, or the committee freezes a copy — and a frozen copy is the committee's table rather than Unicode's, which is the hand-curated normative list [unary-forms](#unary-forms) already priced and declined. The freeze gives up precisely the authority the derivation was borrowing. **The reader cannot see the class:** thirty-two places in the nine blocks put R and B on adjacent code points; ⊎ U+228E is B and ⊏ U+228F is R; ⋇ ⋈ ⋉ run B, R, B. A derived table asks a reader to know which of 1,381 glyphs is which before knowing how an expression groups — the "lattice a reader has to learn" this entry charges against a *declared* fixity, relocated into a lookup rather than avoided. **And the class records typesetting spacing**, which is what MathClass is for; the step from spacing to binding is an inference the committee would be making, not a fact Unicode states. **The concession to make, because it is the objection's strongest form:** `a ≺ b ⊕ c` is `(a ≺ b) ⊕ c`, which has no reading at all — the one place one level is wrong rather than merely surprising. It is normally a *type error*, since a relation returns `bool` and `⊕(bool, c)` does not compile, and that is exactly the difference from `std::cout << a & b`, which compiles silently. 2026-09-23 — **the ODR argument and the implementation argument above are aimed at a *declared* fixity and do not touch a derived table** (design author's ruling). A derived precedence is a pure function of the code point, and clang-format can consult a table — it consults one for every built-in operator — so "the formatter links no Sema" is not an argument against it. The real obstacle is the other half of that sentence: all 1,381 code points share one `tok::TokenKind`, and `Lexer::getUserOperatorCodePoint` recovers the code point at desugaring time, after the precedence query has been asked. But the class is a pure function of the code point, which the lexer already has, so it is two token kinds and a second generated range table; `binops_by_token` on the GCC side is the same story. That is a cost argument, and a small one. The paper should stop deploying the other two against this alternative. 2026-09-23 — **"backwards in a tropical semiring" is wrong, and R1 should cut it.** In the tropical semiring `a ⊕ b` is `min(a, b)` and `a ⊗ b` is `a + b`: ⊗ *is* the multiplication, distributivity runs in the usual direction — `a ⊗ (b ⊕ c)` is `min(a+b, a+c)` — and "multiplication binds tighter than addition" gives the right grouping. What inverts is the glyph-to-arithmetic mapping, ⊗ performing ordinary addition, not the precedence. The third case in the same sentence, a monoidal category whose product is written ⊕, does invert and carries the point on its own. 2026-09-23 — **a third option, not yet analysed: no precedence at all.** Leroy again: make `2*ℤ∪ℕ` ill-formed rather than `2*(ℤ∪ℕ)`, requiring parentheses where a user operator meets an operator of another kind. Ada does this for `and`, `or` and `xor` against each other, and for `**` with itself (Ada RM 4.4, `http://www.ada-auth.org/standards/2yaarm/html/AA-4-4.html`). It sidesteps the fixity argument rather than winning it, and nothing in this log addresses it.
...
- **P4345R1 — the fixity passage is defective and must be repaired.** The one
  item in this section that is a defect in a *shipped* paper rather than a
  question about the design. R0 asserts that nothing in Unicode supports a
  derived precedence, which MathClass-15 falsifies — it is the data file
  behind UTR #25, the report [token-set](#token-set) draws the repertoire
  from — and it offers a tropical-semiring example that does not say what it
  claims. A third option, no precedence at all on the Ada model, has not been
  analysed at all. All three, with their measurements and with what survives
  of the rebuttal, are in [user-declared-fixity](#user-declared-fixity)'s log
  under 2026-09-23. Raised by Robin Leroy.
