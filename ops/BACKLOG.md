...
### hunk-count-provenance

**Severity:** P3.

**Item.** **The Unicode design doc and the Unicode blog post both print the replay audit's *forecast* as the replay's *result*.** [`docs/unicode-operators.md`](../docs/unicode-operators.md) says *"201 of 204 hunks survive onto clean trunk, and 169 of 171 production hunks"*, and [`docs/unicode-infix-operators.org`](../docs/unicode-infix-operators.org) says *"201 of 204 hunks came across unchanged (169 of 171 in the compiler proper)"*. Both are stated as executed fact. They are U19's prediction; U20 executed the replay and landed **200 of 204** and **168 of 171** — its handoff says *"the landed hunk count is 200, not §2's predicted 201"* (hunks that were separate on the branch because a backtick line sat between them coalesce on clean trunk: content, not coverage), and the plan row records "200 hunks (168 production)". The conclusion does not move in either document; only the numbers and their provenance do. *(Found 2026-09-08 by [unicode-paper-truth](completion/steps/unicode-paper-truth.md), which fixed the same defect in the paper and left these two alone because they are outside its scope.)*

**Where.** [`docs/unicode-operators.md`](../docs/unicode-operators.md), the paragraph beginning *"Of the whole 110-file diff"* in the separable-fates section; [`docs/unicode-infix-operators.org`](../docs/unicode-infix-operators.org), the paragraph beginning *"That separability isn't a promise any more"*. The corrected figures and the reason for the difference are in [`ops/unicode-operators/clang/handoffs/U20-upstream-replay.handoff.md`](unicode-operators/clang/handoffs/U20-upstream-replay.handoff.md) and in `ops/unicode-operators/clang/PLAN.md` row U20; the forecast is [`REPLAY.md`](unicode-operators/clang/REPLAY.md) §"Read it three ways".

**Closed by.** *(open)* — a design-doc and blog edit, two paragraphs, no measurement to take: the numbers are already derived and recorded. Worth doing in the same pass as any other edit to either file rather than on its own.
