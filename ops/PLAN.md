...
- [x] **S04** Diagnostics + nested-paren rule ([nesting-vs-chaining](../docs/backtick-operator-design.md#nesting-vs-chaining)) — `ops/steps/04-infix-diagnostics.md` (dep: S03)
...
| F24  | 2026-08-04 | backtick-trunk | 439ceb5237dc, 169e45c7916f | PASS (54107 total, 48212 passed, 27 xfail; 10 failed = 8 known `DirectoryWatcherTest.*` inotify + 2 env-only, both proven name-driven by `CLANG_EXECUTABLE_VERSION=24-backtick`. Filtered rerun: 54099 total, 48212 passed, 2 failed) — two defect fixes, not plan steps: `-ast-print` crash on a non-`CallExpr` semantic form ([wrapper-inner-shape](DEVIATIONS.md#wrapper-inner-shape)) and the static analyzer's five missing sites ([analysis-layer-sites](DEVIATIONS.md#analysis-layer-sites)) | ops/handoffs/15-defect-fixes.handoff.md |
