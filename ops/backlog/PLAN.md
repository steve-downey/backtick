...
|---|---|---|---|---|---|---|
| `backtick-trunk` | 54108 | 48222 | **0** | 27 | 5853 | 6 |
| `backtick-23` | 54342 | 48500 | **1** (`B33` only) | 27 | 5808 | 6 |
| `unicode-operators-experiment` | 54183 | 48295 | **0** | 27 | 5855 | 6 |
| `unicode-operators-upstream` | 54241 | 48323 | **0** | 27 | 5885 | 6 |

**Updated by BL04, 2026-09-03**, from BL01/BL03's figures (trunk 54107 / 5852;
23.x 54341 / 5807; experiment 54181 / 5853; upstream 54240 / 5884). BL04 adds
one `clang/test/CIR/CodeGen/` test per feature — two on the experiment branch,
which carries both — and **every one of them is `Unsupported` in these four
build dirs**, because `clang/test/CIR/lit.local.cfg` sets
`config.unsupported` unless the build defines `CLANG_ENABLE_CIR`. So
discovered and unsupported each move by exactly the number of new test files
and **passed does not move at all**. If you ever configure one of these four
with CIR on, none of this arithmetic holds: see BL04's handoff for the
+44 / −888 / +932 shape it takes instead.
...
- [x] **BL04** `B15`: build ClangIR and close the unknown — `steps/BL04-clangir.md` (dep: BL01) — **also closed `B36`, and reopened `B31`**
...
| BL04 | 2026-09-03 | `unicode-operators-experiment` | `6ee1358f7b47` — 4 CIR arms for `UserOperatorExpr` **and** 4 for `BacktickInfixExpr` (this branch carries both features), +35 production lines across 4 files, + two tests, `clang/test/CIR/CodeGen/unicode-operator.cpp` (+84) and `backtick-infix.cpp` (+63) | **PASS on the real gate — the scratch CIR build, unfiltered, `EXIT=0`, 0 failures**: 54227 discovered / 49227 passed / **0 failed** / 27 XFAIL / 4967 unsupported / 6 skipped. Arithmetic against `build-unicode` on the *same* source tree, re-measured the same day (54183 / 48295 / 0 / 27 / 5855 / 6): discovered **+44** = the `CIRUnitTests` gtest cases (22 lit shards) that exist only with `CLANG_ENABLE_CIR`; unsupported **−888** = −889 (`clang/test/CIR`, 908 discovered, goes from 908 unsupported to 19) **+1** (`Frontend/cir-not-built.c`, which requires CIR *off*); passed **+932** = 889 + 44 − 1. Every term identified, nothing residual. `build-unicode` itself also re-gated at `EXIT=0` (54183 / 48295 / **0** / 27 / 5855 / 6) — BL03's baseline plus the two new tests as `Unsupported`, passed unchanged. **All four probe shapes plus prefix emit CIR instruction-for-instruction identical to the explicit call**, l-value store included. | `handoffs/BL04-clangir.handoff.md` |
| BL04 | 2026-09-03 | `unicode-operators-upstream` | `783a9c1a5f6f` — the `UserOperatorExpr` half only (+24 production, +84 test); the 24 added lines are **byte-identical** to lines compiled and run in the scratch build, verified by diffing the `+` lines of both worktrees before committing | **PASS**: 54241 discovered / 48323 passed / **0 failed** / 27 XFAIL / 5885 unsupported / 6 skipped, `EXIT=0` — BL03's baseline **+1 discovered / +1 unsupported**, passed unchanged, the new test being `Unsupported` without CIR. The CIR behaviour itself was verified in the scratch build by removing the backtick arms, rebuilding and re-running: `unicode-operator.cpp` passes and `backtick-infix.cpp` fails, so the `UserOperatorExpr` half is load-bearing on its own. **U20's headline result survives**: `git diff upstream/main..unicode-operators-upstream \| grep -i backtick` still returns nothing, and the new test contains no backtick reference. | `handoffs/BL04-clangir.handoff.md` |
| BL04 | 2026-09-03 | `backtick-trunk` | `cfbc69be9d4f` — the `BacktickInfixExpr` half only (+21 production, +63 test), closing `B36`; added lines byte-identical to the verified ones | **PASS**: 54108 discovered / 48222 passed / **0 failed** / 27 XFAIL / 5853 unsupported / 6 skipped, `EXIT=0` — BL01's baseline **+1 discovered / +1 unsupported**, passed unchanged. Verified in the scratch build the same way, in reverse: backtick arms alone → `backtick-infix.cpp` passes, `unicode-operator.cpp` fails. | `handoffs/BL04-clangir.handoff.md` |
| BL04 | 2026-09-03 | `backtick-23` | `1cf901e79df4` — same hunks as `backtick-trunk`, applied directly rather than cherry-picked (the anchors are identical on all four branches) | **PASS**: 54342 discovered / 48500 passed / **1 failed** / 27 XFAIL / 5808 unsupported / 6 skipped, `EXIT=1`; the single failure is `Clang :: Format/dump-config-objc-stdin.m`, the documented `backtick-23`-only `B33` artifact — identical to the BL01 baseline **+1 discovered / +1 unsupported**. | `handoffs/BL04-clangir.handoff.md` |
