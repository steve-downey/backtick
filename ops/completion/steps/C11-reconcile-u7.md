# C11 — Reconcile U§7 and §7.1: declaring, using, desugaring
...
**Depends on:** C03 (answered) and C07 — because two of these rows are
questions C03 decides, and the section cannot describe behaviour C07 may be
about to change.
...
- **`DEV-U06` — static member user operators.** Decided by C03. Write the
...
- **`DEV-U10` — operator characters as ordinary names.** Targets §7.1's fourth
  ...
  of `B23`'s wording if C03 decided reword rather than fix: the word
  **"anywhere"** in §7.1 is what `B23` falsifies —
  ...
  C++11. Whichever way C03 went, §7.1 must stop saying "anywhere" without
  qualification.
- **`DEV-U11` — the grammar as built.** Targets §7 "Using" and U§6's "parsing
  is the easy part" claim (C12 owns that sentence; hand it over).
- **`DEV-U15` — the default-argument prefix case.** Decided by C03. If the
  decision was "keep and document", **this step is where the documenting
  happens** — that is why C07 explicitly does not own it.
- **`DEV-U16` — member versus non-member operand sequencing**, decided by
  ...
  which is why C03 did not brief it. Record it as a question the paper asks,
...
1. Read `docs/open-decisions.md` first. Two of these rows are answers now, not
   questions, and writing them as open would undo C03.
...
- No sentence in §7 or §7.1 describes behaviour C07 changed, and none states a
  question C03 answered.
