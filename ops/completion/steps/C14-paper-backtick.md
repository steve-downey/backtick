# C14 — D4307R0, and the blog version
...
**Depends on:** C05 (paper-truth defects fixed), C12 (backtick ledger
reconciled, §17 updated), C13 (parity gaps closed so the paper does not have
to explain them).
...
- **C05's three fixes** change three claims: the flag is now C++-only in fact
  as well as intent (`B03`), the keyword escape round-trips (`B02`), and the
  node's source range spans its operands (`B04`).
- **`DEV-09` (C12)** gives §17 the back-end symmetry sentence: the two
...
- **C01's issue numbers.** Wherever the paper says a defect was found in
  Clang, cite it.
- **C02's WONTFIX reasons**, if `B24` or `B30` went that way — the paper
  should answer those questions rather than let a reviewer raise them.
- **`B12` from C08**: parts of the Clang work have **no GCC counterpart by
...
1. Reconcile the paper against `docs/backtick-operator-design.md`, which C12
