...
### `B03` — `-fbacktick` lacks `ShouldParseIf<cplusplus.KeyPath>` (P2, worst symptom in the file)

The flag changes **C-mode tokenization** for a grammar that is C++-only, and
the re-grade found the symptom is worse than `DEV-U07` recorded: C does not
...
### `B02` — the keyword escape does not round-trip through `-ast-print` (P2)
...
### `B04` — `BacktickInfixExpr`'s source range does not span its operands (P2)
...
- `check-clang` green on both branches against the baselines in
  `ops/completion/PLAN.md`; `backtick-23`'s single `B33` failure is expected
  and nothing else is.
- The C-mode test fails without the `B03` fix. Demonstrate that, do not assume
