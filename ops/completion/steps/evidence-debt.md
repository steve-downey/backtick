...
read that file, it is good, and this one adds `B08` and the reason they belong
together.

**Depends on:** nothing.
**Closes:** `B16`, `B17`, `B21`, `B08`.
**Refs:** `ops/backlog/steps/BL07-unicode-batch.md`; `U06`, `U07`, `U08`,
`U09`, `U11`, `U16`, `U17` handoffs.

## Why these four are one step

Not because they share a branch — `B08` is on the backtick side — but because
...
### `B16` — the lldb hunk is compile-unverified (P2)
...
### `B17` — code-completion priority grouping never updated (P2)
...
### `B21` — the UCD 17.0.0 inputs are in neither repo (P3, but reproducibility)
...
"refresh" anything: U1 is frozen at 17.0.0 by design, and the manifest exists
to prove the frozen bytes are the ones published.

### `B08` — no `-ast-print` test covers a backtick expression in a template context (P3)
...
1. `B16` first — it needs the build reconfigure, so start it and do the others
   while it builds.
2. `B21` next if the network is up; it is the one that can become impossible.
3. `B17` and `B08` are ordinary test work.
