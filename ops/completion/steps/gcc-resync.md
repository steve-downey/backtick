...
**Depends on:** nothing.
**Closes:** `B09`, `B10`, `B11`, `B12`, `B13`; reconciles `DEV-G08`.
...
### 1. `B13` — re-sync to current GCC trunk
...
### 2. `B09` — pure ADL on a template-id slot (P2, paper-truth)
...
Note the neighbourhood: `DEV-G05` was the ADL defect `G10` fixed with an
explicit `perform_koenig_lookup` on a bare-name slot. This is the same defect
one grammar production over.

### 3. `B10` — module streaming of keyword-escaped names (P2, deferred three times)
...
### 4. `B11` — the `flag_backtick` guard in `grokdeclarator` is over-permissive (P3)
...
- `B09`'s and `B10`'s new tests fail before their fixes where a fix was made,
  and are recorded as *verified-not-broken* where none was needed.
- Five `Closed by` cells filled; `DEV-G08` marked.
