...
- One feature-test macro is proposed for the paper's two features, per
  their joint design (D14). If they are ever polled separately, it splits
  into `__cpp_backtick_operator` and `__cpp_escaped_identifiers`.
...
>
> [2]{.pnum} When parsing a *backtick-operator*, the first non-nested
