...
```bash
probes/escape-positions.sh              # the coverage table
probes/escape-positions.sh --verbose    # with the diagnostic for each rejection
probes/escape-errors.sh                 # exit 1 if anything hung or was accepted
probes/flag-off-parity.sh               # exit 1 if the flag changed anything
CLANG=~/src/llvm/build-backtick/bin/clang++ probes/escape-positions.sh
```
