// src/examples/ranges/calendar.pipe.cpp                             -*-C++-*-
// SPDX-License-Identifier: Apache-2.0 WITH LLVM-exception
//
// A calendar, after Eric Niebler's range-v3 example
// (range-v3/example/calendar.cpp, Boost Software License 1.0), which in turn
// follows H. S. Teoh's article on component programming with ranges. The
// stage structure is his; the port is to std::ranges and std::chrono, so
// boost::gregorian, boost::format and the hand-written interleave_view are
// gone. It prints 2015, three months to a line, and the output is
// byte-identical to the one documented in the original.
//
// It is here because it is the largest ranges pipeline anyone points at: six
// stages at the top level and three more nested inside them. If the backtick
// spelling only reads well on two-stage examples, this is where that shows
// up. calendar.backtick.cpp is the same pipeline spelled infix, and the two
// must print the same thing.
//
// The std::format calls are ordinary functions rather than lambdas on
// purpose. basic_format_string has a consteval constructor, and a lambda
// that calls one becomes immediate-escalating under P2564; clang then
// rejects the whole view chain. Nothing to do with the backtick -- this file
// does not use it.
...
auto dates(int start_year, int stop_year) {
    ...
    return views::iota(0, (last - first).count()) |
           views::transform([first](int i) { return first + days{i}; });
}
...
const auto format_week = [](auto week) {
    ...
    const auto text = week | views::transform(format_day) | views::join |
...
const auto layout_month = [](auto month) {
    auto lines = month | views::chunk_by(same_week) |
                 views::transform(format_week) | std::ranges::to<std::vector>();
...
const auto transpose = [](auto months) {
    return views::iota(0uz, month_lines) |
           views::transform([months](std::size_t row) {
               return months | views::transform(
                                   [row](const std::vector<std::string> &m) {
                                       return m[row];
                                   });
           });
};
...
const auto join_row = [](auto row) {
    auto line = row | views::join | std::ranges::to<std::string>();
...
// 2cd3ee41-bdaf-4ed4-a1f2-d6c22ad258cb
int main() {
...
// 2cd3ee41-bdaf-4ed4-a1f2-d6c22ad258cb end
